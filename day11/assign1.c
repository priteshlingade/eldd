...
static int irq;
static struct work_struct workque;
...
static void work_handler(struct work_struct *workque) {
	int i=0;
    while(i<10)
    {
    led_state = !led_state;
	gpio_set_value(LED_GPIO, led_state);	
    printk(KERN_INFO "%s: workque_handler() called.\n", THIS_MODULE->name);
    mdelay(1000);
    i++;
    }
}
...
static __init int bbb_gpio_init(void) {
	...
	// initialize tasklet
	INIT_WORK(&workque,work_handler);
