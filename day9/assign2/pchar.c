...
static __init int pchar_init(void)
{
	...
	// semaphore init 
	sema_init(&s,1);
	printk(KERN_INFO "%s : sema_init() -  semaphore is initilized \n", THIS_MODULE->name);
...
static int pchar_open(struct inode *pinode, struct file *pfile)
{
	printk(KERN_INFO "%s : pchar_open() is called on begale bone\n", THIS_MODULE->name);
	// decreament the semaphore count 
	down(&s);
	printk(KERN_INFO "%s: process start semaphore down() on begale bone .\n", THIS_MODULE->name);
...
static int pchar_close(struct inode *pinode, struct file *pfile)
{
	printk(KERN_INFO "%s : pchar_close() is called on beagel bone\n", THIS_MODULE->name);
	// increament the semaphore count 
	up(&s);
	printk(KERN_INFO "%s: process complete semaphore up() on beagle bone.\n", THIS_MODULE->name);
